            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-slate-200 text-left">
                  {['Medicine', 'Generic', 'Stock', 'Level', 'Actions'].map((h) => (
                    <th
                      key={h}
                ...
                {inventory.map((inv) => (
                  <tr key={inv.id} className="border-b border-slate-100 hover:bg-slate-50">
                    ...
                    <td className="px-4 py-3">
                      ...
                      <span className="text-xs text-slate-400"> / {inv.threshold}</span>
                    </td>
      ...
      >
        <div className="space-y-4">
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-2">
              {(['ADD', 'REDUCE', 'SET'] as StockAction[]).map((a) => (
                ...
                >
                  {a === 'ADD' ? 'Add' : a === 'REDUCE' ? 'Reduce' : 'Set exact'}
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="rounded-lg border border-blue-200 bg-blue-50 px-3 py-2.5 text-sm text-blue-800">
            New stock level: <span className="font-semibold">{preview} units</span>
          </div>
        </div>
      </Modal>
    </>
